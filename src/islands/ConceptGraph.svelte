<script>
  ...
  // Portal's lexicon force-graph. Layout is frozen at build time (ADR-0008); this renders plain SVG,
  // color-coded by domain, with typed edges (integral-of / derivative-of / …). Click a node to see its
  // formula and its sourced relations.
...
<div class="cg">
  <div class="canvas">
    <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label="Formula concept graph">
      <defs>
      ...
      {#each graph.nodes as n}
        <g class="node" class:dim={dimmed(n.id)} onclick={() => (selected = selected === n.id ? null : n.id)}
           role="button" tabindex="0" onkeydown={(ev) => ev.key === "Enter" && (selected = selected === n.id ? null : n.id)}>
          <circle cx={n.x} cy={n.y} r={radius(n)} class={domClass(n.domain)} class:sel={selected === n.id} />
          <text x={n.x} y={n.y + 5} text-anchor="middle" class="nlabel">{n.label}</text>
        </g>
      {/each}
    </svg>
  </div>
  ...
  <aside class="panel">
    ...
    {:else}
      <p class="muted">Click a node to see the formula and its typed relations. Green arrows are the
        calculus spine: <em>x is the integral of v is the integral of a</em>; reverse them and you have
        the derivatives.</p>
      <ul class="legend">
